...
(function ($) {
    ...
    if ($autoplay == "autoplay") {
	  ...
	  var $covepartnerplayer_template = "<div class='video-wrap' style='width:100%; padding-top: 62.5%; position:relative;'><iframe class='partnerPlayer' frameborder='0' marginwidth='0' marginheight='0' scrolling='no' width='100%' height='100%' style='position:absolute; top:0;' src='http://video.pbs.org/widget/partnerplayer/COVEPLAYERID/?start=0&end=0&chapterbar=false&endscreen=false" + $coveplayer_chrome + $coveplayer_autoplay +"'></iframe></div>";
  ...
  function receiveVideoPBSMessage(event){
    console.log(event);
    if ( event.origin == 'http://video.pbs.org' ) {
      if (! VideoPBSMessagesRecieved) {
  ...
  function WriteOutYoutubePlayer(youtubeid, divid, playervars){
    ...
    $("#" + ytplayerid).wrap("<div class='video-wrap' style='width:100%; padding-top: 62.5%; position:relative;'></div>");
