...
class COVE_Asset_Manager {
  ...
  public function do_daily_episode_generate() {
    ...
    $create_episode = get_option('coveam_mm_episode_autocreate') ? strtolower(get_option('coveam_mm_episode_autocreate')) : false;

    //regen the season list, optionally creating a new season if episodes will be autogenerated
    $season_resp = $this->update_media_manager_season_list($create_episode); 
  ...
  public function update_media_manager_season_list($create_current_year_season_if_not_found = false) {
    ...
    if (($create_current_year_season_if_not_found == 'true') && !$current_year_found ) {
      $created_season_id = $client->create_child($show_id, 'show', 'season', array('ordinal' => $current_year));
      if (!$created_season_id || !empty($created_season_id['errors'])) {
        error_log("COVE Asset Manager attempted to create season $current_year but failed with error: " . json_encode($created_season_id['errors']));
      } else {
        $newseason = array('value' => $created_season_id, 'label' => $current_year);
        array_unshift($seasonary, $newseason);
      }
    }
    $result = false;
    if (!empty($seasonary)) {
