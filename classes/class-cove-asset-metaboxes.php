...
class COVE_Asset_Metaboxes {
  ...
  public function setup_custom_scripts() {
    wp_enqueue_script( 'youtube_cors', $this->assets_url . 'js/youtube_cors.js', array( 'jquery' ), 3.6, true );
  ...
  public function get_episode_option_list($monthnum = 0, $year = 0) {
    ...
    foreach($this_option_query->get_posts() as $this_episode_post) {
      ...
      $html .= "<option value='". $thiscid . "'>" . $this_title . "</option>";
    }
    wp_reset_postdata();
    return $html;
  }
  ...
  public function ajax_get_youtube_access_token() {
    ...
    $google_access_token = $wnet_youtube_obj->get_google_access_token();
    if (!$google_access_token) {
      error_log("ajax request for google access token failed");
    }
